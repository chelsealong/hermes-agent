...
class TestKeylessFailover:
    ...
    def test_search_fails_over_on_pinned_vendor_403(self, monkeypatch):
        # Reproduces #113892: a pinned keyless Firecrawl 403 (IP-reputation block) must
        # not disable the whole ring when a peer vendor is healthy.
        self._pin(monkeypatch, "firecrawl")
        monkeypatch.setitem(
            keyless_mcp._KEYLESS_SEARCHERS, "firecrawl",
            lambda q, l: {
                "success": False,
                "error": "Keyless Firecrawl search failed: Client error '403 Forbidden' for url "
    ...
    def test_extract_fails_over_when_all_urls_403(self, monkeypatch):
        self._pin(monkeypatch, "firecrawl")
        forbidden = [
            {"url": "https://a", "title": "", "content": "", "error": "Client error '403 Forbidden' for url 'https://a'."},
            {"url": "https://b", "title": "", "content": "", "error": "Client error '403 Forbidden' for url 'https://b'."},
        ]
        good = [
            {"url": "https://a", "title": "A", "content": "x"},
            {"url": "https://b", "title": "B", "content": "y"},
