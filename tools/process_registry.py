...
class ProcessRegistry(ProcessCheckpointMixin):
    ...
    def _move_to_finished(self, session: ProcessSession):
        ...
        The claim (popping from ``_running`` and inserting into ``_finished``)
        happens atomically under one lock acquisition so only one caller proceeds
        AND ``get()``/``is_session_waiting()`` can never observe the session as
        absent from both maps. ``save_completed_result``'s disk I/O runs outside
        the lock -- the lock must only guard in-memory state, never a filesystem
        write, or every ``get()``/``is_session_waiting()`` call from the gateway's
