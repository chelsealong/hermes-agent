...
class GatewaySlashCommandsMixin(
    ...
    async def _handle_yolo_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        # The central slash gate can allow selected commands to non-admin users, so enforce admin
        # again at this side-effect boundary. Unconfigured policies remain unrestricted.
        policy = policy_for_source(self.config, event.source)
        if not policy.is_admin(event.source.user_id):
            return "Only gateway admins can enable YOLO mode."
        session_key = self._session_key_for_source(event.source)
