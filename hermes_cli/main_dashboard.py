...
def _loaded_launchd_backend_jobs(
    ...
    for kind, plist_dir in (plist_dirs if plist_dirs is not None else _launchd_plist_dirs()):
        ...
        for plist_path in plists:
            ...
            # expat raises ExpatError for XML that is not well-formed (e.g. a hand-edited plist
            # with a raw `&` in a <string>); ExpatError is not a ValueError, so leaving it out let
            # one bad plist abort the whole scan instead of being skipped like the docstring says.
